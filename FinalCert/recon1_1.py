...
CHASE_FOLDER = r'CHASE Path'
DB_FOLDER = r'DB Path'
SUMMARY_REPORT_PATH = r'SUMMARY Path'
IMPORT_FOLDER_PATH = r'IMPORT Path' 
ACCUMULATIVE_BASE_DIR = r'ACCUMULATIVE PATH'
...
def format_adds_for_import(adds_df, client_type, today_date_str):
    ...
    if client_type == 'Chase':
        ...
        # --- Added Feature: Translate DB Doc Status Codes ---
        ...
        if raw_status is not None and hasattr(raw_status, 'map'):
            ...
            # Clean spaces for exact mapping criteria, fallback to original if not in dictionary
